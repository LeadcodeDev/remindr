...
use crate::{
    ...
    app::{
        ...
        states::{
            app_state::AppState,
            database_state::{DatabaseState, LoadedDatabaseView, OpenedDatabaseView},
...
impl DatabaseViewScreen {
    ...
    fn load_view_if_needed(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        let (needs_loading, key, database_id) = cx.read_global::<DatabaseState, _>(|state, _| {
        ...
        if needs_loading {
            if let (Some(key), Some(database_id)) = (key, database_id) {
    ...
    fn get_or_create_table_view(
        ...
        let entity = cx.new(|_| TableView::new(key, database_id));
        ...
        match current_view {
            Some(view) => match &view.state {
                ...
                LoadingState::Loaded(_) => {
                    let table_view =
                        self.get_or_create_table_view(view.unique_key(), view.database_id, cx);
                    div()
                        .bg(cx.theme().background.lighten(0.2))
                        .flex()
                        .flex_col()
